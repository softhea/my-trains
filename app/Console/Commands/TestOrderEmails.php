...
class TestOrderEmails extends Command
{
    ...
    public function handle()
    {
        ...
        if ($orderId) {
            $order = Order::with(['orderProduct', 'orderSeller', 'orderBuyer'])->find($orderId);
            if (!$order) {
            ...
            $order = Order::with(['orderProduct', 'orderSeller', 'orderBuyer'])->latest()->first();
            if (!$order) {
        ...
        $this->info("Buyer: {$order->orderBuyer?->name} ({$order->orderBuyer?->email})");
        $this->info("Seller: {$order->orderSeller?->name} ({$order->orderSeller?->email})");
        $this->info("Product: {$order->orderProduct?->name}");

        if ($type === 'new' || $type === 'both') {
            $this->info("\n--- Testing New Order Notification (to Seller) ---");
            if (!$order->orderSeller?->email) {
                $this->warn("⚠️ No seller email available for this order.");
            } else {
                try {
        ...
        if ($type === 'accepted' || $type === 'both') {
            ...
            if (!$order->orderBuyer?->email) {
                $this->warn("⚠️ No buyer email available for this order.");
            } else {
                try {
                    Mail::to($order->orderBuyer->email)->send(new OrderAcceptedNotification($order));
